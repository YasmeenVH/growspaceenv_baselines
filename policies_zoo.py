...
def dqn_stable_baselines_training():
    wandb.run = config.tensorboard.run
    wandb.tensorboard.patch(save=False, tensorboardX=True)
    ...
    model = DQN("CnnPolicy", envs, verbose=1, tensorboard_log="./runs/", clip_range=0.2, n_steps=50)
    model.learn(total_timesteps=1000000, log_interval=1, callback=WandbStableBaselines3Callback())
    model.save(f"{config.env_name}_stable_baselines_ppo")

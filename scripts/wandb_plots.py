...
def compare_mnist(data):
    ...
    fig, ax = plt.subplots(1)
    #.xticks('Mnist Digits')
    ax.set_ylabel('Average Episode Reward',fontsize=16)
    ax.set_xlabel('Mnist digits',fontsize=16)
    ax.boxplot(data_plot,labels=labels, patch_artist=True)
    #plt.xticks('Mnist Digits')
    #plt.yticks('Average Episode Reward')
    plt.show()
...
if __name__ == "__main__":
    ...
    if analysis=='hierarchy':
        ...
        hierarchy_general = ["growspace/hierarchy/1em80av5", "growspace/hierarchy/175xlpwl",
                             "growspace/hierarchy/sb1uoeaj"]
        hierarchy_easy = ["growspace/growspaceenv_baselines/23gkjv7q", "growspace/growspaceenv_baselines/3pyhttvi",
                          "growspace/growspaceenv_baselines/252dmds9"]
        hierarchy_hard = ["growspace/growspaceenv_baselines/2t5jolss", "growspace/growspaceenv_baselines/2kvijwmx",
                          "growspace/growspaceenv_baselines/38vgv98h"]

        hard_oracle = [5,5,5]
        hard_random = [0.74,0.74,0.74]
        easy_oracle = [8.95,8.95,8.95]
        easy_random = [2.8,2.8,2.8]

        r_easy_smooth, r_easy_smooth1 = extract_(hierarchy_easy, 2)
        r_hard_smooth, r_hard_smooth1 = extract_(hierarchy_hard, 2)
        ...
        fig, ax = plt.subplots(1)
        ax.plot(x, easy, color='red')
        ax.fill_between(x, easy + err_e, easy - err_e, alpha=0.3, color='red', label='PPO Easy')
        ax.plot(x, hard, color='blue')
        ax.fill_between(x, hard + err_h, hard - err_h, alpha=0.3, color='blue', label='PPO Hard')

        x_oracle = np.linspace(0, 1000000, 3)
        ax.plot(x_oracle, easy_oracle, color='red', linestyle='dotted', label='Easy Oracle',linewidth=1.5)
        ax.plot(x_oracle, easy_random, color='red', linestyle='dashed', label='Easy Random',linewidth=1.5)

        ax.plot(x_oracle, hard_oracle, color='blue', linestyle='dotted', label='Hard Oracle',linewidth=1.5)
        ax.plot(x_oracle, hard_random, color='blue', linestyle='dashed', label='Hard Random',linewidth=1.5)
        ...
        ax.legend(loc='lower center',fontsize=18, ncol=3, bbox_to_anchor=(0.5,-0.3))
        #ax.legend(loc='center left', bbox_to_anchor=(1, 0.5), fontsize='small')
        # plt.plot(x, rewards_mean)
        plt.show()
    if analysis =='gen':
        ppo  = ["growspace/control/2uw60tub","growspace/control/xufs0mnh","growspace/control/2k8ug0gs"]
        a2c = ["growspace/growspaceenv_baselines/1w4r8ljy","growspace/growspaceenv_baselines/1aezj3aj","growspace/growspaceenv_baselines/1osy0g48"]
        rainbom = []
        r_ppo,r_easy_smooth1 = extract_(ppo, 5)
        r_a2c, r_hard_smooth1 = extract_(a2c, 5)
        # for i in range(len(r_easy_smooth)):
        #     print(len(r_easy_smooth[i]))
        #     print(len(r_hard_smooth[i]))
        all_control = [r_ppo,r_a2c]
        ...
        fig, ax = plt.subplots(1)
        ax.plot(x, easy, color='red')
        ax.fill_between(x, easy + err_e, easy - err_e, alpha=0.3, color='red', label='PPO')
        ax.plot(x2,hard,color='blue')
        ax.fill_between(x2, hard+ err_h, hard - err_h,alpha=0.3, color='blue',label='A2C')
    ...
    if analysis=='control':
        ...
        # control_easy =["growspace/control/n2dzahl6","growspace/control/2i9wb6bu"]#,"growspace/growspaceenv_baselines/3n0dlkzl"]
        # control_hard =["growspace/control/85prsz2s","growspace/control/27jlk0zv","growspace/control/1b3ym56i"]
        control_easy =["growspace/growspaceenv_baselines/1qjxpr9f","growspace/growspaceenv_baselines/4ceiwshv","growspace/growspaceenv_baselines/1en3miwm"]#,"growspace/growspaceenv_baselines/3n0dlkzl"]
        control_hard =["growspace/growspaceenv_baselines/3fc8hit1","growspace/growspaceenv_baselines/2z9eg3wk","growspace/growspaceenv_baselines/16axingl"]

        hard_oracle = [6.5,6.5,6.5]
        hard_random = [1.05,1.05,1.05]
        easy_oracle = [11.2,11.2,11.2]
        easy_random = [3.68,3.68,3.68]


        r_easy_smooth,r_easy_smooth1 = extract_(control_easy, 5)
        r_hard_smooth, r_hard_smooth1 = extract_(control_hard, 5)
        # for i in range(len(r_easy_smooth)):
        #     print(len(r_easy_smooth[i]))
        #     print(len(r_hard_smooth[i]))
    ...
    if analysis=='fairness':
        ...
        # fairness_middle =["growspace/fairness/2a0aa0hj","growspace/fairness/cugl7h51","growspace/fairness/1vkbn7na"]
        # fairness_above =["growspace/fairness/g84ktb8c","growspace/fairness/1tgon55v","growspace/fairness/27yv69rl"]
        fairness_easy =["growspace/growspaceenv_baselines/1vm5kdpy","growspace/growspaceenv_baselines/220gbck1","growspace/growspaceenv_baselines/1uw64ys1"]
        fairness_middle =["growspace/growspaceenv_baselines/20fvfxqd","growspace/growspaceenv_baselines/1gek1qhs","growspace/growspaceenv_baselines/1m05ikye"]
        #fairness_above =["growspace/growspaceenv_baselines/68iy6y58","growspace/growspaceenv_baselines/1us32crc","growspace/growspaceenv_baselines/3j2lbj08"]
        #above_oracle =[2,2,2]
        #above_random =[0.89,0.89,0.89]
        middle_oracle =[3.2,3.2,3.2]
        middle_random =[1.11,1.11,1.11]
        easy_oracle =[4.2,4.2,4.2]
        easy_random =[1.5,1.5,1.5]

        r_easy_smooth,r_easy_smooth1 = extract_(fairness_easy, 3)
        r_middle_smooth, r_middle_smooth1 = extract_(fairness_middle, 3)
        #r_above, r_above_smooth = extract_(fairness_above, 5)

        #smooth_r = moving_average(rewards, window)
        #all_smooth_r.append(smooth_r)

        all_fair = [r_easy_smooth,r_middle_smooth]
        ...
        ax.fill_between(x, middle + err_m, middle - err_m,alpha=0.3, color='blue',label='PPO Hard')
        ...
        ax.plot(x_oracle,middle_oracle,color='blue', linestyle='dotted',label='Hard Oracle',linewidth=1.5)
        ax.plot(x_oracle, middle_random, color='blue', linestyle='dashed',label='Hard Random',linewidth=1.5)
    ...
    if analysis=='mnistmix':
        ...
        mnist0 =["growspace/mnistexperiments/e4q9dnu6","growspace/mnistexperiments/3vlmbhqm","growspace/mnistexperiments/1b6k8vfn"]
        mnist3 =["growspace/mnistexperiments/m27wfa1x","growspace/mnistexperiments/11o1sv4w","growspace/growspaceenv_baselines/2clvl10i"]
        r0, mnist00 = extract_(mnist0, 3)
        r3, mnist33 = extract_(mnist3, 3)
        mnist_curriculum = ["growspace/mnistexperiments/t1f7ovsx","growspace/mnistexperiments/4o5ulndv"]

        r_easy,r_easy_smooth = extract_(mnistmix, 3)
        r_hard, r_hard_smooth = extract_(mnist_curriculum, 2)
        # for i in range(len(r_easy_smooth)):
        #     print(len(r_easy_smooth[i]))
        #     print(len(r_hard_smooth[i]))
        #all_control = [r_easy_smooth,r_hard_smooth]
        all_control = [r_easy,r_hard]
        control, err = data_viz(all_control)
        easy, err_e = control[0],err[0]
        hard, err_h = control[1],err[1]
        ...
        fig, ax = plt.subplots(1)
        ax.plot(x, easy, color='red')
        ax.fill_between(x, easy + err_e, easy - err_e, alpha=0.3, color='red', label='PPO Mixed Digits')
        ax.plot(x2,hard,color='blue')
        ax.fill_between(x2, hard+ err_h, hard - err_h,alpha=0.3, color='blue',label=' PPO Curriculum')
